...
client_cmd='printf "\033]0;mygosh client\007"; sleep 0.5; echo "client: go run ./bin connect localhost:42022 \"hello world\""; go run ./bin connect localhost:42022 "hello world"; status=$?; echo; echo "client exited with status $status"; exec "${SHELL:-/bin/sh}"'
